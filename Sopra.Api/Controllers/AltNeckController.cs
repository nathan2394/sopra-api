...
namespace Sopra.Api.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class AltNeckController : ControllerBase
    {
        private readonly IServiceAsync<AltNeck> _service;

        public AltNeckController(IServiceAsync<AltNeck> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "AltNeckController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<AltNeck>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AltNeck obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] AltNeck obj)
        {
            try
            {
