...
namespace Sopra.Api.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class AltNeckDetailController : ControllerBase
    {
        private readonly IServiceAsync<AltNeckDetail> _service;

        public AltNeckDetailController(IServiceAsync<AltNeckDetail> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "AltNeckDetailController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<AltNeckDetail>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AltNeckDetail obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] AltNeckDetail obj)
        {
            try
            {
