...
namespace Sopra.Api.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class AltVolumeController : ControllerBase
    {
        private readonly IServiceAsync<AltVolume> _service;

        public AltVolumeController(IServiceAsync<AltVolume> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "AltVolumeController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<AltVolume>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AltVolume obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] AltVolume obj)
        {
            try
            {
