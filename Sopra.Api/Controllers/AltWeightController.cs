...
namespace Sopra.Api.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class AltWeightController : ControllerBase
    {
        private readonly IServiceAsync<AltWeight> _service;

        public AltWeightController(IServiceAsync<AltWeight> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "AltWeightController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<AltWeight>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AltWeight obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] AltWeight obj)
        {
            try
            {
