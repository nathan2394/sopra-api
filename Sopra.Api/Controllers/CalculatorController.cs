...
namespace Sopra.Api.Controllers
{
    ...
    {
        private readonly IServiceAsync<Calculator> _service;

        public CalculatorController(IServiceAsync<Calculator> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "CalculatorController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<Calculator>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Calculator obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] Calculator obj)
        {
            try
            {
