...
namespace Sopra.Api.Controllers
{
    ...
    {
        private readonly CartInterface _service;

        public CartController(CartInterface service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "CartController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<Cart>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Cart obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] Cart obj)
        {
            try
            {
        ...
        public async Task<IActionResult> ChangeCart(int id, int customerId)
        {
            try
            {
                var result = await _service.ChangeCart(id, customerId);
