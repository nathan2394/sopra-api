...
namespace Sopra.Api.Controllers
{
    ...
    {
        private readonly IServiceAsync<District> _service;

        public DistrictController(IServiceAsync<District> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "DistrictController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<District>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] District obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] District obj)
        {
            try
            {
