...
namespace Sopra.Api.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class PromoQuantityController : ControllerBase
    {
        private readonly IServiceAsync<PromoQuantity> _service;

        public PromoQuantityController(IServiceAsync<PromoQuantity> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "PromoQuantityController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<PromoQuantity>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PromoQuantity obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] PromoQuantity obj)
        {
            try
            {
