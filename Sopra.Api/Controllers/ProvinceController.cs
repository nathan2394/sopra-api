...
namespace Sopra.Api.Controllers
{
    ...
    {
        private readonly IServiceAsync<Province> _service;

        public ProvinceController(IServiceAsync<Province> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "ProvinceController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<Province>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Province obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] Province obj)
        {
            try
            {
