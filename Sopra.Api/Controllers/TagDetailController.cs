...
namespace Sopra.Api.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class TagDetailController : ControllerBase
    {
        private readonly IServiceAsync<TagDetail> _service;

        public TagDetailController(IServiceAsync<TagDetail> service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            ...
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "TagDetailController");
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                ...
                var response = new Response<TagDetail>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] TagDetail obj)
        {
            try
            {
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] TagDetail obj)
        {
            try
            {
