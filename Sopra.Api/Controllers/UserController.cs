...
namespace Sopra.Api.Controllers
{
    ...
    {
        private readonly IServiceAsync<User> _service;

        public UserController(IServiceAsync<User> service)
        {
            _service = service;
        }
        ...
        //[Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] User obj)
        {
            try
            {
        ...
        public async Task<IActionResult> ChangePassword([FromBody] ChangePassword obj, int id)
        {
            try
            {
                var result = await _service.ChangePassword(obj,id);
                if (result == null)
                    return BadRequest(new { message = "Invalid ID" });
