...
{
    public class BillingAddressService : IServiceAsync<BillingAddress>
    {
        private readonly EFContext _context;

        public BillingAddressService(EFContext context)
        {
            _context = context;
        }

        public async Task<BillingAddress> CreateAsync(BillingAddress data)
        {
            await using var dbTrans = await _context.Database.BeginTransactionAsync();
            try
            {
                data.RefID = 0;
                await _context.BillingAddresses.AddAsync(data);
                await _context.SaveChangesAsync();

                // Check Validate
                await Utility.AfterSave(_context, "BillingAddress", data.ID, "Add");
        ...
        public async Task<bool> DeleteAsync(long id, long userID)
        {
            ...
            try
            {
                var obj = await _context.BillingAddresses.FirstOrDefaultAsync(x => x.ID == id && x.IsDeleted == false);
                ...
                // Check Validate
                await Utility.AfterSave(_context, "BillingAddress", id, "Delete");
        ...
        public async Task<BillingAddress> EditAsync(BillingAddress data)
        {
            await using var dbTrans = await _context.Database.BeginTransactionAsync();
            try
            {
                var obj = await _context.BillingAddresses.FirstOrDefaultAsync(x => x.ID == data.ID && x.IsDeleted == false);
                ...
                // Check Validate
                await Utility.AfterSave(_context, "BillingAddress", data.ID, "Edit");
        ...
        public Task<BillingAddress> GetAccIdAsync(long id, long customerid)
        {
            throw new NotImplementedException();
        }

        public async Task<ListResponse<BillingAddress>> GetAllAsync(int limit, int page, int total, string search, string sort, string filter, string date)
        {
            try
            {
                _context.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
                var query = from a in _context.BillingAddresses where a.IsDeleted == false select a;
                ...
                return new ListResponse<BillingAddress>(data, total, page);
            }
            catch (Exception ex)
            {
                Trace.WriteLine(ex.Message);
        ...
        public async Task<BillingAddress> GetByIdAsync(long id)
        {
            try
            {
                return await _context.BillingAddresses.AsNoTracking().FirstOrDefaultAsync(x => x.ID == id && x.IsDeleted == false);
            }
            catch (Exception ex)
            {
                Trace.WriteLine(ex.Message);
        ...
        Task<BillingAddress> IServiceAsync<BillingAddress>.ChangePassword(ChangePassword obj, long id)
        {
            throw new NotImplementedException();
        }
